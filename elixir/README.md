...
- `/stats` — all-time project/task accounting, active sessions, service uptime, and per-worker rate limits
...
`GET /api/v1/state` exposes the same snapshot under `stats`. Task responses expose aggregate
